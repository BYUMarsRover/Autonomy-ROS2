...
OPTIONS="$(getopt --name launch --options hvu:t:l:w:a:d: --longoptions help,verbose,base-env,rover-env,user:,task:,location:,wait:,attach:,docker-port: -- "$@")"
...
if [ -z "$DOCKER_SSH_PORT" ]; then
  DEFAULT_DOCKER_SSH_PORT='2233'
  DOCKER_SSH_PORT=$DEFAULT_DOCKER_SSH_PORT
fi
...
# TODO enter the docker container and source the environment
echo 'Need to Develop to enter the docker container'

# Source the rover workspace, if it has been built, warn the user that it has not been built otherwise
BASE_REPO_SETUP="$BASE_STATION_REPO/rover_ws/devel/setup.sh"
...
# Decide whether to use rosmon or roslaunch
# rosmon is preferred for more coherent output
LAUNCHER='rosrun rosmon_core rosmon'
rospack find rosmon_core &> /dev/null || {
  printWarning "Falling back to roslaunch instead of rosmon
Please install rosmon with: sudo apt install ros-melodic-rosmon
"
  LAUNCHER='roslaunch --wait'
}
...
tmux send-keys "$SET_ROVER_ENV_CMD && $LAUNCHER start \
  rover_task_${TASK_NAME}.launch" Enter
...
tmux send-keys "$SET_BASE_ENV_CMD && $LAUNCHER start \
  base_task_${TASK_NAME}.launch" Enter
