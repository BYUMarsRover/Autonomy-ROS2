...
def sift_calculate(first_image,second_image,path_to_save_result,orb_create_matches=300,min_match_count = 10, flann_index_lsh= 6):
...
def orb_calculate(first_image,second_image,path_to_save_result,orb_create_matches=300,min_match_count = 10, flann_index_lsh= 6):
    start_time = time.time()
    ...
    elapsed_time = end_time - start_time
    print(f"time for image reading: {image_read_time-start_time}")
    print(f"time for detecting and computing ORB keypoints and descriptors: {orb_detect_and_compute_time-image_read_time}")
