...
class EKFNode(Node):
    def __init__(self):
        ...
        self.timer = self.create_timer(0.05, self.ekf_loop)  # 20 Hz loop
    
    def declare_ekf_parameters(self):
        self.declare_parameter('gps_noise', 0.4)  # 0.2 m
        self.declare_parameter('imu_noise', 0.01)  # 0.1 rad
        self.declare_parameter('odom_noise', 5.5) # 2.0 m
    ...
    def ekf_loop(self):
        ...
        with self.state_lock:
            ...
            if self.new_odom and self.start_filter:
                if self.new_imu:
    ...
    def publish_odom_map_frame(self, msg: Odometry):
        try:
            ...
            odom_map.header.stamp = self.get_clock().now().to_msg()
