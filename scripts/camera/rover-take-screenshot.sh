usage() {
  echo "Usage:
          rover-take-screenshot.sh

        Takes screenshot on camera

        Camera cannot be busy (streaming video to base station). If so, this
        script will not be able to take a screenshot.
        ...
        Examples:
          ./rover-take-screenshot.sh -d /dev/video0 -s 4;
          ./rover-take-screenshot.sh -d gripperCam -s 3;
          "
  exit 2;
}
...
FILE="BYU-Mars-Rover/rover_ws/src/science/src/presentation/resources/${SITE}-$(date +%s).png"
...
! filesink location="$HOME/${FILE}" sync=false async=false
